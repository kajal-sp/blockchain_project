...
const models = require('./src/models/block.model');
...
exports.AuditLogBlockchain = class AuditLogBlockchain {
  ...
  async addBlockToChain(blockInfo) {
    ...
    const chainInfo = models.AuditLogChain();
  ...
  async getGenesisBlock() {
    const blockInfo = await models.AuditLogChain.find().sort({ $natural: 1 }).limit(1);
